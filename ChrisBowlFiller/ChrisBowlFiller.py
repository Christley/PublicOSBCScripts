...
class OSRSChrisBowlFiller(WillowsDadBot):
    def __init__(self):
        bot_title = "Chris Bowl Filler"
        description = "Go to Falador west bank with empty bowls in the bank."
        ...
        while time.time() - self.start_time < self.end_time:
            ...
            try:
                ...
                if len(self.api_m.get_inv_item_indices(ids.BOWL)) == 0:
                    ...
                    self.close_bank()
                self.log_msg("Starting to fill bowls")
                self.fill_bowls()
                time.sleep(self.random_sleep_length())
                if self.api_m.get_run_energy() >= 9500:
    ...
    def sleep(self, percentage):
        ...
        while len(self.api_m.get_inv_item_indices(ids.BOWL)) != 0:
    ...
    def fill_bowls(self):
        self.mouse.move_to(self.win.inventory_slots[random.randint(0, 7)].random_point())
        self.mouse.click()
        time.sleep(self.random_sleep_length())
        self.log_msg("Locating a tap")
        self.locate_tap()
        time.sleep(self.random_sleep_length())

    def locate_tap(self):
        """
        This function will locate the tap and click.
        """
        if tap := self.get_all_tagged_in_rect(self.win.game_view, clr.PINK):
            tap = sorted(tap, key=RuneLiteObject.distance_from_rect_center)
            if len(tap) > 0:
                self.log_msg("Found a tap.")
                self.mouse.move_to(tap[0].random_point())
                self.log_msg("Using on the pump.")
                self.mouse.click()
                time.sleep(self.random_sleep_length())
        else:
            self.log_msg("No taps found, quitting bot.")
            self.stop()
